...
export default defineConfig({
	...
	// In CI, only run Chromium (since that's all that's installed)
	...
	webServer: {
		// In CI, the build is already done by the workflow (build:static:github)
		// Locally, we need to build before preview
		command: process.env.CI
			? "ASTRO_OUTPUT=static pnpm preview"
			: "ASTRO_OUTPUT=static pnpm build:static && ASTRO_OUTPUT=static pnpm preview",
